...
typedef enum numOrVar {none, num, var, varnum} nOV;
...
/*carries out the doo loop functionality*/
unsigned char dooloop(userprog* p, SDL_Simplewin* sw, short min, short max, char letter);
/*checks words after failed instruction reads to check they're valid*/
void checkFailedInsWords(userprog* p);
/*checks when a new line occurs*/
unsigned char newLineCheck(userprog* p);
/*checks for VALID instruction CMD*/
void checkInstruction(userprog* p);
/*prints users code to console*/
